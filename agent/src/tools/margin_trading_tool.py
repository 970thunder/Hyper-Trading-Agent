...
class MarginTradingTool(BaseTool):
    ...
    def execute(self, **kwargs: Any) -> str:
        ...
        try:
            ...
            try:
                fallback = tushare_fallbacks.fetch_margin_trading(code, days=days)
                return json.dumps({"ok": True, "market": "a_share", "source": "tushare", "warnings": [f"eastmoney failed ({exc}); used tushare fallback"], "data": fallback}, ensure_ascii=False)
        ...
        if not isinstance(data, list) or not data:
            try:
                fallback = tushare_fallbacks.fetch_margin_trading(code, days=days)
                return json.dumps({"ok": True, "market": "a_share", "source": "tushare", "warnings": ["eastmoney returned no rows; used tushare fallback"], "data": fallback}, ensure_ascii=False)
            except Exception as fallback_exc:
