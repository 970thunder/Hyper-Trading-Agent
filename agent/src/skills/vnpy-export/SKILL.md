---
name: vnpy-export
description: Export a Hyper-Trading-Agent backtest strategy to a runnable vnpy CtaTemplate Python class — supports A-share equities, futures, and crypto via BarGenerator + ArrayManager.
category: tool
---

## Overview

This skill translates a Hyper-Trading-Agent strategy into a **vnpy `CtaTemplate` subclass** `.py` file
...
run a Hyper-Trading-Agent strategy inside vnpy's CTA backtester or live trading engine.
...
class {{StrategyName}}Strategy(CtaTemplate):
    """
    Hyper-Trading-Agent export — {{StrategyName}}
    Generated from run: {{run_id}}
    Instrument: {{vt_symbol}}
    """

    author = "Hyper-Trading-Agent"
...
| Python (Hyper-Trading-Agent / pandas / ta-lib) | vnpy ArrayManager |
|----------------------------------------|-------------------|
...
| Hyper-Trading-Agent signal | Position check | vnpy call |
|---------------------|---------------|-----------|
...
When the Hyper-Trading-Agent strategy uses multiple timeframes (e.g., daily signal, hourly entry):
...
- [ ] Comment block at top of file notes the original Hyper-Trading-Agent `run_id` and instrument
