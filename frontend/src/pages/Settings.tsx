...
import { api, isAuthRequiredError, type ChannelRuntimeStatus, type DataSourceSettings, type KnowledgeDocument, type KnowledgeStats, type LLMProviderOption, type LLMSettings } from "@/lib/api";
...
export function Settings() {
    ...
    Promise.allSettled([
      ...
      .then(([llmResult, dataSourceResult, knowledgeResult, channelResult]) => {
        ...
        if (knowledgeResult.status === "fulfilled") {
          ...
          const message = knowledgeResult.reason instanceof Error ? knowledgeResult.reason.message : "Unknown error";
          toast.error(`Failed to load knowledge base status: ${message}`);
        }
  ...
  const submitKnowledge = async (event: FormEvent) => {
    ...
    try {
      ...
      toast.error(`Failed to index knowledge document: ${error instanceof Error ? error.message : "Unknown error"}`);
    } finally {
  ...
  if (loading || !form || !settings || !dataSettings || !knowledgeStats) {
    return (
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <form onSubmit={submitKnowledge} className="rounded-lg border bg-card p-5 shadow-sm">
        <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
            ...
            <p className="max-w-3xl text-sm text-muted-foreground">{"Index local text documents for RAG retrieval. The agent can call knowledge_search before answering domain-specific questions."}</p>
          </div>
          <button
            type="button"
            onClick={() => refreshKnowledge().catch((error) => toast.error(`Failed to refresh knowledge base: ${error instanceof Error ? error.message : "Unknown error"}`))}
        ...
        <div className="mb-5 grid gap-3 md:grid-cols-3">
          <div className="rounded-md border bg-muted/20 px-3 py-2">
            ...
            <div className="text-sm font-medium">{knowledgeStats.document_count}</div>
          </div>
          <div className="rounded-md border bg-muted/20 px-3 py-2">
            <div className="text-xs text-muted-foreground">{"Chunks"}</div>
            <div className="text-sm font-medium">{knowledgeStats.chunk_count}</div>
          </div>
          <div className="rounded-md border bg-muted/20 px-3 py-2">
            <div className="text-xs text-muted-foreground">{"Storage"}</div>
            <div className="truncate text-sm font-medium" title={knowledgeStats.db_path}>{knowledgeStats.db_path}</div>
          </div>
        </div>

        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(220px,0.45fr)_auto]">
          <label className="grid gap-2">
            <span className={labelClass}>{"Document path"}</span>
            <input
              value={knowledgePath}
              onChange={(event) => setKnowledgePath(event.target.value)}
              className={fieldClass}
              placeholder={"uploads/research-notes.md"}
            />
          </label>
          <label className="grid gap-2">
          ...
          <button
            type="submit"
            disabled={knowledgeSaving || !knowledgePath.trim()}
            className="inline-flex items-center justify-center gap-2 self-end rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-70"
          >
            {knowledgeSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
            {"Index"}
          </button>
        </div>
