...
class LiveAuthorizeRequest(BaseModel):
    ...
    Hyper-Trading-Agent never holds funds and never operates a venue, so the OAuth
...
def register_live_routes(
    ...
    async def live_authorize_endpoint(payload: LiveAuthorizeRequest):
        ...
        return {
            ...
            "instruction": (
                f"Run `hyper-trading connector authorize {connector_profile}` "
                "from the device that will hold the broker session. This opens the "
                "broker's own OAuth consent flow; Hyper-Trading-Agent never holds funds and "
