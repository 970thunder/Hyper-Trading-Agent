...
The contract this file defends:
  ...
  * Without the env var, ``~/.hyper-trading-agent/swarm-agent.json`` (the swarm-
    specific operator file) is preferred over ``agent.json``.
  * Without either swarm-specific file, ``~/.hyper-trading-agent/agent.json``
...
real ``~/.hyper-trading-agent`` directory of whoever is running the suite.
"""
