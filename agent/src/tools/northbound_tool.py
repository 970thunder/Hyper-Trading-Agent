...
class NorthboundFlowTool(BaseTool):
    ...
    def execute(self, **kwargs: Any) -> str:
        ...
        try:
            ...
            try:
                fallback = tushare_fallbacks.fetch_northbound_flow(lookback_days=lookback_days)
                return json.dumps({"ok": True, "market": "China A", "source": "tushare", "warnings": [f"eastmoney failed ({exc}); used tushare fallback"], "data": fallback}, ensure_ascii=False)
            except Exception as fallback_exc:
                return json.dumps({"ok": False, "error": f"{exc}; tushare fallback failed: {fallback_exc}"}, ensure_ascii=False)
