...
flowchart LR
  ...
  G -- 否 --> H
  G -- 是 --> O[模拟盘 / 受控实盘前验证]
  A[Hyper Trading Agent] -. 编排、审计、检索、报告 .-> D
  A -.-> F
  A -.-> B
  A -.-> R
```
...
- **绝对收益**：策略本身的收益。
- **超额收益**：策略收益减去基准收益，\(r_t^{excess}=r_t-r_t^{benchmark}\)。
...
\[
Sharpe=\frac{E[r-r_f]}{\sigma(r-r_f)}\sqrt{N}
\]
...
\[
Sortino=\frac{E[r-r_f]}{\sigma_{downside}(r-r_f)}\sqrt{N}
\]
...
设净值历史最高点为 \(H_t=\max(V_0,\ldots,V_t)\)，当前回撤为：

\[
DD_t=\frac{V_t}{H_t}-1
\]

**最大回撤（Maximum Drawdown, MDD）**是整个区间中最深的 \(DD_t\)。它回答“从峰值到谷值最惨时亏了多少”，是实务中比单纯波动率更直观的风险指标。
...
一个因子在日期 \(t\) 对股票 \(i\) 的值可以写为 \(f_{i,t}\)。研究通常检验它与未来 \(h\) 期收益 \(r_{i,t\rightarrow t+h}\) 的关系。
...
\[
IC_t = corr(f_{i,t}, r_{i,t\rightarrow t+h})
\]
...
2. **标准化（Z-score）**：\(z=(x-\mu)/\sigma\)，便于不同因子比较。
...
相关性矩阵把多个资产、因子或策略两两之间的相关系数排成方阵。若有 \(n\) 个对象，就得到 \(n\times n\) 矩阵：
...
\[
Cov(i,j)=\rho_{ij}\sigma_i\sigma_j
\]
