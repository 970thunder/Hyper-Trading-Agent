...
class BaseEngine(ABC):
    ...
    def _execute_bars(
        ...
        for i, ts in enumerate(dates):
            ...
            # Rebalances execute at the bar open. Do not size orders using the
            # current close because that price is not observable yet.
            equity = self._calc_open_equity(data_map, close_df, ts)
            target_weights: Dict[str, Optional[float]] = {}
            for c in codes:
                try:
                    target_weights[c] = float(target_pos.at[ts, c]) if ts in target_pos.index else 0.0
                except Exception as exc:
                    target_weights[c] = None
                    logger.warning("Target weight failed for %s at %s: %s", c, ts, exc)

            # Close first so rotations release capital before any replacement
            # order is considered. This makes execution independent of codes.
            for c in codes:
            ...
            # Price the whole opening basket before mutating capital. If the
            # basket does not fit after fees and lot rounding, scale every
            # target uniformly so no early symbol receives preferential fills.
            ...
            # Hooks use current-bar close data. An opening signal that exits at
            # the open must therefore run before a close-based liquidation.
            for c in codes:
                if ts in data_map[c].index:
                    self.on_bar(c, data_map[c].loc[ts], ts)

            # Record end-of-bar equity after market-specific hooks.
    ...
    def _calc_open_equity(
        ...
        """Value positions with prices observable when this bar opens."""
        ...
        for symbol, pos in self.positions.items():
            ...
            unrealized = self._calc_pnl(symbol, pos.direction, pos.size, pos.entry_price, current_price)
    ...
    def _plan_open_order(
        ...
        """Price an opening order without changing portfolio state."""
        self._active_symbol = symbol
    ...
    def _execute_open_order(self, order: _OpenOrder, ts: pd.Timestamp) -> None:
        """Commit a previously priced order after the basket is approved."""
